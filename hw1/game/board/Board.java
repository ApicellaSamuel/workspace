...
public interface Board<P> {
    ...
    /** Ritorna true se p è una posizione della board.
     * @param p  una posizione
     * @return true se p è una posizione della board
     * @throws NullPointerException se p è null */
    default boolean isPos(Pos p) {
        if (p == null) throw new NullPointerException("The position is null");
        
        for(Pos pos : positions()) if(pos.equals(p))  return true;
    ...
    default Set<Pos> get() {
        Set<Pos> occPositions = new HashSet<Pos>();
        
        for (Pos position : positions())
        	
        	if( get(position) != null) occPositions.add(position);

        
        return Collections.unmodifiableSet(occPositions);
    }
     ...
     * {@link Board#get(Object)} e {@link Board#get(Pos)}.
    ...
    default Set<Pos> get(P pm) {
    	Set<Pos> occPositions = get();
    	Set<Pos> occPositionsPm = new HashSet<Pos>();
    	if (pm == null) throw new NullPointerException("The model is null");
    	for(Pos position : occPositions)
    		
    		if(pm.equals(get(position))) occPositionsPm.add(position);
    		
    	return Collections.unmodifiableSet(occPositionsPm);
    }
     ...
     * @param p  una posizione
     * @param d  una direzione
     * @param n  un numero di posizioni
     * @throws UnsupportedOperationException se questa board è immodificabile
    ...
    default void put(P pm, Pos p, Dir d, int n) {
        if(!isModifiable())
        	...
        	throw new UnsupportedOperationException("The boardgame is unmodifiable");
