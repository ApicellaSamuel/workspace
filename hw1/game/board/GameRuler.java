...
public interface GameRuler<P> {
    ...
    default boolean isValid(Move<P> m) {
    	if(m == null) throw new NullPointerException("The parameter is null");
    	if(result() != -1) throw new IllegalStateException("The game is over");
     ...
     * {@link GameRuler#valimove.actions.get(0).kind!=Kind.ADDmove.actions.get(0).kind!=Kind.ADDdMoves()}.
    ...
    default Set<Move<P>> validMoves(Pos p) {
    	...
    	if(!getBoard().positions().contains(p)) throw new IllegalArgumentException("The position doesnt exist");
    	if(result() != -1) throw new IllegalStateException("The game is over");  
        Set<Move<P>> allValidMoves = validMoves();
        Set<Move<P>> validMoves = new HashSet<Move<P>>();
        for (Move<P> move : allValidMoves)
           	if(move.actions.get(0).pos.get(0).equals(p) ) validMoves.add(move);
